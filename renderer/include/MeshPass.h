...
// Maybe one can also stack "syncing" actions by specifying the next pipeline
// stage? Something like: manager.sync(handle, VK_PIPELINE_STAGE_COLOR...) Then
// it will automatically generate and write the required barrier Maybe read this
// next: https://www.sctheblog.com/blog/vulkan-synchronization/
//
// From the article:
// vkCmdDraw(). Used to render triangles to attachments. Usually does something
// in stages for *_VERTEX_SHADER_BIT and *_FRAGMENT_SHADER_BIT. Depth and
// stencil operations (both tests and writes) are in stages for
// *_EARLY_FRAGMENT_TESTS_BIT and *_LATE_FRAGMENT_TESTS_BIT. We have already
// seen
...
public:
  ...
  MeshPass(ImageManager &imageManager)
      : manager(imageManager) {}

  ~MeshPass() override {}

  void setAttachments(ImageHandle color, ImageHandle depth) {
    colorHandle = color;
    depthHandle = depth;
  }

  void setDrawItems(std::vector<DrawItem>* newItems) override { items = newItems ; }
  ...
  void beginRendering(VkCommandBuffer cmd) {
    ...
    manager.sync(cmd, depthHandle,
                 VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT |
                     VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT,
                 VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_WRITE_BIT |
                     VK_ACCESS_2_DEPTH_STENCIL_ATTACHMENT_READ_BIT);

    //std::cout << "Using extent with " << manager.swapExtent.height << " x "
    //          << manager.swapExtent.width << "\n";
    ...
    depthStencilAttachmentInfo.sType =
