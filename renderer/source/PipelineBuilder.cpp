...
void PipelineBuilder::disableDepthWrite() {
  depthStencil.depthWriteEnable = VK_FALSE;
}
...
void PipelineBuilder::enablePushConstants(size_t size, VkShaderStageFlags stageFlag) {
...
VkShaderModule
PipelineBuilder::createShaderModule(const std::vector<char> &code) {
...
void PipelineBuilder::setVertexFormat(
  ...
  vertexInputInfo.pVertexAttributeDescriptions =
      &vertexInputAttributes[0]; // attribes goes out of scope after function
                                 // returns
...
void PipelineBuilder::setAttachmentFormats(uint32_t attachmentCount,
...
void PipelineBuilder::build() {
  ...
  pipelineLayoutInfo.pSetLayouts =
      descriptorSetLayouts.data(); // ADD UI DESCRIPTOR SET LAYOUT
  std::cout << "Created pipeline with setLayoutCount = "
            << descriptorSetLayouts.size() << "\n";
  pipelineLayoutInfo.pPushConstantRanges = &pushConstant;
  pipelineLayoutInfo.pushConstantRangeCount = pushConstantRangeCount;

  if (vkCreatePipelineLayout(context->device, &pipelineLayoutInfo, nullptr,
                             &bundle.layout) != VK_SUCCESS) {
